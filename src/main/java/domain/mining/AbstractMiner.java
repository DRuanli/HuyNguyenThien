...
public abstract class AbstractMiner {
    ...
    /**
     * Parallelization mode controlling which parts run in parallel.
     * IMMUTABLE: Set at construction, never changes.
     */
     ...
     */
    /**
     * Constructor with custom support calculator and parallelization mode.
     *
     * @param database uncertain database to mine
     * @param tau probability threshold (0 < τ ≤ 1)
     * @param k number of top patterns to find (≥ 1)
    ...
    public AbstractMiner(UncertainDatabase database, double tau, int k,
                         SupportCalculator calculator, ParallelizationMode mode) {
        ...
        // Validate parallelization mode
        if (mode == null) {
            throw new IllegalArgumentException("ParallelizationMode cannot be null");
        }
    ...
    /**
     * Constructor with custom support calculator (parallelization mode defaults to DEFAULT).
     *
     * @param database uncertain database to mine
     * @param tau probability threshold (0 < τ ≤ 1)
     * @param k number of top patterns to find (≥ 1)
    ...
    public AbstractMiner(UncertainDatabase database, double tau, int k,
        ...
        this(database, tau, k, calculator, ParallelizationMode.DEFAULT);
    }

    /**
     * Constructor without custom calculator (deprecated - prefer 5-parameter version).
     *
     * @param database uncertain database to mine
     * @param tau probability threshold (0 < τ ≤ 1)
     * @param k number of top patterns to find (≥ 1)
    ...
    public AbstractMiner(UncertainDatabase database, double tau, int k) {
        ...
        this.vocab = database.getVocabulary();
        this.parallelizationMode = ParallelizationMode.DEFAULT;
        this.cache = new HashMap<>();
    }
     ...
     * @return The parallelization mode (never null after construction)
     */
    protected final ParallelizationMode getParallelizationMode() {
    ...
    protected List<FrequentItemset> computeAllSingletonSupports() {
        ...
        // Create stream (parallel or sequential based on parallelization mode)
        java.util.stream.IntStream stream = java.util.stream.IntStream.range(0, vocabSize);
        if (parallelizationMode.isPhase1Parallel()) {
            stream = stream.parallel();  // Parallel processing
        }
        // Otherwise stream remains sequential
     ...
     * </ol>
     *
     * <p>This method dispatches to either sequential or parallel implementation based on
     * parallelization mode.</p>
     *
    ...
    /**
     * Sequential implementation of 1-itemset closure checking.
     *
     * @param oneItemFI The 1-itemset to check
     * @param supOneItem The support of the 1-itemset
     * @param frequent1Itemset The list of all 1-itemsets in support-descending order
     ...
     * @return True if the 1-itemset is closed, false otherwise
     */
    private boolean checkClosure1ItemsetSequential(FrequentItemset oneItemFI, int supOneItem,
                                                   List<FrequentItemset> frequent1Itemset, int minsup) {
    ...
    /**
     * Parallel implementation of 1-itemset closure checking.
     *
     * <p><b>Parallelization Strategy:</b></p>
     * <ul>
     *   <li>Use parallel stream to check closure against all other items concurrently</li>
     *   <li>Short-circuit as soon as closure violation is found (using .anyMatch())</li>
     *   <li>Thread-safe cache writes using ConcurrentHashMap (already in place)</li>
     * </ul>
     *
     * <p><b>Advantages:</b></p>
     * <ul>
     *   <li>Speeds up closure checking for large vocabularies (100+ items)</li>
     *   <li>Each item pair (A, B) checked independently in parallel</li>
     *   <li>Expected speedup: 1.5-2x on 4-core systems</li>
     * </ul>
     *
     * @param oneItemFI The 1-itemset to check
     * @param supOneItem The support of the 1-itemset
     ...
     * @return True if the 1-itemset is closed, false otherwise
     */
    private boolean checkClosure1ItemsetParallel(FrequentItemset oneItemFI, int supOneItem,
                                                 List<FrequentItemset> frequent1Itemset, int minsup) {
        // Extract the item ID from the singleton itemset
        int itemA = oneItemFI.getItems().get(0);

        // Parallel closure checking: check if ANY other item violates closure
        // .anyMatch() short-circuits as soon as violation is found
        boolean violated = frequent1Itemset.parallelStream()
            .filter(otherFI -> {
                int itemB = otherFI.getItems().get(0);
                 ...
                 * If other item's support < current item's support,
                 * it cannot violate closure (anti-monotonicity).
                 */
                if (otherFI.getSupport() < supOneItem) return false;

                return true;  // Keep this item for closure checking
            })
            .anyMatch(otherFI -> {
                // Create the union itemset A ∪ B
                Itemset unionItemset = oneItemFI.union(otherFI);
                ...
                if (cached != null) {
                    ...
                    /**
                     * Cache the 2-itemset if the other item meets minsup.
                     * ConcurrentHashMap ensures thread-safe cache writes.
                     */
                    if (otherFI.getSupport() >= minsup) {
                        cache.put(unionItemset, new CachedFrequentItemset(unionItemset, supAB, probAB, tidsetAB));
                    }
                ...
                /**
                 * Closure violation check:
                 * If support(A ∪ B) == support(A), then A is not closed.
                 * anyMatch() returns true, short-circuiting remaining checks.
                 */
                return supAB == supOneItem;
            });

        // Return true if NOT violated (i.e., is closed)
        return !violated;
    }
    ...
    /**
     * Sequential implementation of closure checking and extension generation.
     *
     * @param candidate The candidate pattern to check
     * @param threshold The current minimum support threshold
     * @return ClosureCheckResult containing closure status and valid extensions
     */
    private ClosureCheckResult checkClosureAndGenerateExtensionsSequential(FrequentItemset candidate, int threshold) {
    ...
    /**
     * Parallel implementation of closure checking and extension generation.
     *
     * <p><b>Parallelization Strategy:</b></p>
     * <ul>
     *   <li>Use parallel stream to process all frequent items concurrently</li>
     *   <li>Thread-safe closure flag using AtomicBoolean</li>
     *   <li>Concurrent cache access using ConcurrentHashMap</li>
     *   <li>Collect all valid extensions in parallel</li>
     * </ul>
     *
     * <p><b>Advantages:</b></p>
     * <ul>
     *   <li>Speeds up extension generation for candidates with many potential extensions</li>
     *   <li>Each extension computed independently in parallel</li>
     *   <li>Expected speedup: 1.5-2.5x on 4-core systems for deep searches</li>
     * </ul>
     *
     * <p><b>Note:</b> Only used when frequentItemCount > 50 to avoid overhead on small itemsets.</p>
     *
     * @param candidate The candidate pattern to check
     * @param threshold The current minimum support threshold
     * @return ClosureCheckResult containing closure status and valid extensions
     */
    private ClosureCheckResult checkClosureAndGenerateExtensionsParallel(FrequentItemset candidate, int threshold) {
        int supX = candidate.getSupport();

        // Thread-safe closure flag (AtomicBoolean for concurrent updates)
        AtomicBoolean isClosed = new AtomicBoolean(true);

        // Get maximum item in X for canonical order enforcement
        int maxItemInX = getMaxItemIndex(candidate);

        // Parallel extension generation using IntStream
        List<FrequentItemset> extensions = IntStream.range(0, frequentItemCount)
            .parallel()  // ← Parallel processing
            .mapToObj(idx -> {
                int item = frequentItems[idx];

                // Skip if item already in candidate itemset
                if (candidate.contains(item)) return null;
                 ...
                 */
                if (itemSupport < threshold) {
                    return null;  // Extension cannot enter Top-K
                }

                /**
                 * Determine what operations we need to perform
                 */
                boolean needClosureCheck = (itemSupport >= supX) && isClosed.get();
                boolean needExtension = (item > maxItemInX);
                ...
                /**
                 * Skip if neither closure check nor extension needed
                 */
                if (!needClosureCheck && !shouldGenerateExtension) {
                    return null;
                }
                ...
                if (cached != null) {
                    ...
                    // Cache the computed result (thread-safe ConcurrentHashMap)
                    cache.put(Xe, new CachedFrequentItemset(Xe, supXe, probXe, tidsetXe));
                }

                /**
                 * Closure check: If support(Xe) == support(X), mark as not closed
                 * Use AtomicBoolean.compareAndSet for thread-safe update
                 */
                if (needClosureCheck && supXe == supX) {
                    isClosed.set(false);
                }
                 ...
                 * Generate extension if needed
                 */
                if (shouldGenerateExtension && supXe >= threshold) {
                    return new FrequentItemset(Xe, supXe, probXe);
                }

                return null;  // No extension generated
            })
            .filter(Objects::nonNull)  // Remove null entries
            .collect(Collectors.toList());

        return new ClosureCheckResult(isClosed.get(), extensions);
    }
